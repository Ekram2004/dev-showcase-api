...
router.get('/', authenticateToken, authorizeRole(['admin']), async (req, res, next) => {
  try {
    const allUsers = await query('SELECT u.id, u.username, u.email, u.bio, u.github_url, linkedin_url, u.portfolio_url, u.profile_picture_url, r.name as role FROM users u JOIN roles r ON u.role_id = r.id ORDER BY u.id ASC');
...
router.get('/:id', async (req, res, next) => {
  const { id } = req.params;
  try {
    const user = await query('SELECT u.id, u.username, u.email, u.bio, u.github_url, u.linkedin_url, u.portfolio_url, u.profile_picture_url, r.name as role_name, u.created_at, u.updated_at FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = $1', [id]);
...
router.put('/:id', authenticateToken, authorizeOwnership('users', 'id', 'id'), [
  ...
  const { id } = req.params;
  const { username, email, password, bio, github_url, linkedin_url, portfolio_url, profile_picture_url } = req.body;
  try {
    const fields = [];
    const params = [id];
...
router.delete('/:id', authenticateToken, authorizeOwnership('users', 'id', 'id'), async (req, res, next) => {
  const { id } = req.params;
  try {
...
router.put('/:id/role', authenticateToken, authorizeRole(['admin']), [
  body('roleName')
    ...
    .isIn(['user', 'admin']).withMessage('Invalid role name.')
], handleValidationErrors, async (req, res, next) => {
  const { id } = req.params;
  ...
  try {
    ...
    res.json({ message: `User ${updatedUser.rows[0].username} role updated to ${roleName}.` });
