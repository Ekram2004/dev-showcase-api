...
router.get("/:id", async (req, res, next) => {
  const { id } = req.params;
  try {
    const skill = await query("SELECT id, name FROM skills WHERE id = $1", [
      id,
    ]);
...
router.put(
  ...
  async (req, res, next) => {
    ...
    try {
      const existingSkill = await query(
        "SELECT id FROM skills WHERE name = $1 AND id != $2"[(name, id)],
      );
      if (existingSkill.rows.length > 0) {
        const conflictError = new Error("Skill with this name already exists.");
        conflictError.statusCode = 409;
      ...
      const updatedSkill = await query(
        "UPDATE skills SET name = $1 WHERE id = $2 RETURNING id, name",
        [name, id],
      );
...
router.delete(
  ...
  async (req, res, next) => {
    const { id } = req.params;
    try {
      const deletedSkill = await query(
