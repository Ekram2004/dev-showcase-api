...
router.post('/register', [
    ...
    body('password').notEmpty().withMessage('Password required').isLength({ min: 6 }).withMessage('Password must be at least 6 charcters')
], handleValidationErrors, async (req, res, next) => {
    const { username, email, password } = req.body;
    try {
        const existsingUser = await query('SELECT id FROM users WHERE username = $1 OR email = $2', [username, email]);
        ...
        if (roleResult.rows[0].length === 0) {
            const serverError = new Error('Internal server error or roles not found');
...
router.post("/refresh-token", [
    ...
    try {
        const existingRefreshToken = await query('SELECT rt.*, u.username , r.name as role_name FROM refresh_tokens rt JOIN users u ON rt.user_id = u.id JOIN roles r ON u.role_id = r.id WHERE rt.token = $1 AND rt.revoked = FALSE AND rt.expires_at > NOW()', [refreshToken]);
...
router.post('/logout', [
    body('refreshToken').trim().notEmpty().withMessage('Refresh token is required logout.')
], handleValidationErrors, async (req, res, next) => {
    const { refreshToken } = req.body;

    try {
        const result = await query('UPDATE refresh_tokens SET revoked = TRUE WHERE token = $1 RETURNING id', [refreshToken]);

        if (result.rows.length === 0) {
            console.warn('Logout : Refresh token not found or already revoked:', refreshToken);
        }
